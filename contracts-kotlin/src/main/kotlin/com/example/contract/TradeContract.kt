...
class TradeContract : Contract {
    ...
    override fun verify(tx: LedgerTransaction) {
        ...
        requireThat {
            ...
            "No inputs should be consumed when issuing a Trade." using (tx.inputs.isEmpty())
            "Only one output state should be created." using (tx.outputs.size == 1)
            val out = tx.outputsOfType<TradeState>().single()
            "The buyer and the seller cannot be the same entity." using (out.buyer != out.seller)
            "All of the participants must be signers." using (command.signers.containsAll(out.participants.map { it.owningKey }))

            // Trade-specific constraints.
            "The Trade's price must be non-negative." using (out.price > 0)
        }
    }
